# Câu hỏi thường gặp về Hosting

> **Danh mục:** Dịch vụ Hosting
> **Cập nhật lần cuối:** 2026-08-14
> **Người phụ trách:** Phòng kỹ thuật
> **Từ khóa:** hosting, web hosting, lưu trữ website, tên miền, SSL, băng thông

## Tổng quan

Tài liệu này tổng hợp các câu hỏi thường gặp của khách hàng về dịch vụ hosting (lưu trữ website) do công ty cung cấp, bao gồm đăng ký, kỹ thuật, thanh toán và hỗ trợ.

## Đăng ký và gói dịch vụ

**Hỏi: Hosting là gì?**
Trả lời: Hosting là dịch vụ cho thuê không gian lưu trữ trên máy chủ (server) để khách hàng đặt dữ liệu website (mã nguồn, hình ảnh, cơ sở dữ liệu...) giúp website hoạt động và truy cập được trên Internet 24/7.

**Hỏi: Công ty có những gói hosting nào?**
Trả lời: Công ty cung cấp nhiều gói hosting phù hợp với từng nhu cầu: gói Cá nhân (website nhỏ, blog), gói Doanh nghiệp (website công ty, bán hàng vừa và nhỏ), và gói Cao cấp (website có lượng truy cập lớn, thương mại điện tử). Mỗi gói khác nhau về dung lượng lưu trữ, băng thông và số lượng website/email đi kèm.

**Hỏi: Làm sao để đăng ký hosting?**
Trả lời: Khách hàng có thể đăng ký trực tiếp trên website công ty https://tgs.com.vn/premium-hosting bằng cách chọn gói hosting phù hợp, điền thông tin thanh toán, hoặc liên hệ hotline 0919 768 789  để được tư vấn và hỗ trợ đăng ký.
![TheGioSo](https://cms.tgs.com.vn/wp-content/uploads/2026/07/LOGO-THE-GIOI-SO-2024_CAM.png)

**Hỏi: Tôi có thể nâng cấp hoặc hạ cấp gói hosting đang dùng không?**
Trả lời: Có. Khách hàng có thể yêu cầu nâng cấp hoặc hạ cấp gói hosting bất kỳ lúc nào thông qua trang quản lý dịch vụ hoặc liên hệ hỗ trợ kỹ thuật. Chênh lệch chi phí sẽ được tính theo thời gian còn lại của gói hiện tại.

**Hỏi: Hosting có bao gồm tên miền không?**
Trả lời: Tên miền và hosting là hai dịch vụ riêng biệt. Một số gói khuyến mãi có tặng kèm tên miền miễn phí năm đầu, khách hàng nên kiểm tra chi tiết khuyến mãi của từng gói hoặc liên hệ tư vấn để biết chính xác.

## Kỹ thuật và cấu hình

**Hỏi: Hosting hỗ trợ những hệ quản trị nội dung (CMS) nào?**
Trả lời: Hosting của công ty hỗ trợ cài đặt các CMS phổ biến như WordPress, Joomla, Magento thông qua công cụ cài đặt tự động (Softaculous) có sẵn trong bảng điều khiển (cPanel/DirectAdmin), giúp khách hàng dựng website nhanh chóng mà không cần thao tác thủ công.

**Hỏi: Tôi có được cấp bảng điều khiển quản lý hosting không?**
Trả lời: Có. Mỗi gói hosting đều được cấp tài khoản quản trị (cPanel hoặc DirectAdmin tùy gói) để khách hàng tự quản lý file, cơ sở dữ liệu, email, sao lưu và các thiết lập kỹ thuật khác.

**Hỏi: Hosting có hỗ trợ chứng chỉ SSL không?**
Trả lời: Có. Tất cả các gói hosting đều được tích hợp SSL miễn phí (Let's Encrypt) giúp website chạy giao thức HTTPS an toàn. Khách hàng cũng có thể mua và cài đặt chứng chỉ SSL trả phí nếu có nhu cầu bảo mật cao hơn.

**Hỏi: Băng thông và dung lượng lưu trữ của hosting có giới hạn không?**
Trả lời: Có, mỗi gói hosting có giới hạn dung lượng lưu trữ và băng thông riêng, được nêu rõ trong bảng giá dịch vụ. Nếu website vượt quá giới hạn, khách hàng sẽ nhận được thông báo để nâng cấp gói phù hợp.

**Hỏi: Dữ liệu website có được sao lưu (backup) không?**
Trả lời: Có. Hệ thống tự động sao lưu dữ liệu định kỳ hàng ngày hoặc hàng tuần tùy gói dịch vụ. Khách hàng cũng có thể tự tạo bản sao lưu thủ công qua bảng điều khiển bất cứ lúc nào.

**Hỏi: Tôi có thể tự cài đặt phần mềm hoặc mã nguồn riêng lên hosting không?**
Trả lời: Có, khách hàng có toàn quyền upload mã nguồn, cài đặt ứng dụng thông qua FTP hoặc File Manager trong bảng điều khiển, miễn là phù hợp với tài nguyên và điều khoản sử dụng của gói hosting.

## Chuyển dữ liệu và di chuyển website

**Hỏi: Tôi đang dùng hosting của nhà cung cấp khác, có thể chuyển sang hosting công ty không?**
Trả lời: Có. Công ty hỗ trợ chuyển dữ liệu (migration) miễn phí từ nhà cung cấp khác sang hosting của công ty, bao gồm mã nguồn, cơ sở dữ liệu và email. Khách hàng liên hệ bộ phận kỹ thuật để được hỗ trợ chuyển đổi mà không làm gián đoạn website đang chạy.

**Hỏi: Chuyển hosting có làm website bị gián đoạn không?**
Trả lời: Quá trình chuyển hosting được thực hiện song song, website cũ vẫn hoạt động bình thường cho đến khi dữ liệu mới được xác nhận hoạt động tốt trên hosting mới, sau đó mới trỏ tên miền sang. Vì vậy thời gian gián đoạn (nếu có) thường rất ngắn, chủ yếu do thời gian cập nhật DNS.

## Thanh toán và gia hạn

**Hỏi: Hosting thanh toán theo chu kỳ nào?**
Trả lời: Hosting có thể thanh toán theo tháng, theo năm hoặc theo chu kỳ nhiều năm tùy gói. Thanh toán theo năm hoặc nhiều năm thường được chiết khấu ưu đãi hơn so với thanh toán theo tháng.

**Hỏi: Nếu quên gia hạn, hosting và website có bị mất dữ liệu không?**
Trả lời: Khi hosting hết hạn, website sẽ tạm ngưng hoạt động nhưng dữ liệu vẫn được giữ trong một khoảng thời gian gia hạn (thường 15-30 ngày) trước khi bị xóa hoàn toàn. Công ty sẽ gửi email/thông báo nhắc gia hạn trước khi hết hạn để khách hàng chủ động xử lý.

**Hỏi: Có được hoàn tiền nếu không hài lòng với dịch vụ hosting không?**
Trả lời: Công ty áp dụng chính sách hoàn tiền trong 7 ngày đầu sử dụng nếu khách hàng không hài lòng với dịch vụ, với điều kiện chưa vi phạm điều khoản sử dụng. Liên hệ bộ phận CSKH để được hướng dẫn thủ tục hoàn tiền.

## Hỗ trợ kỹ thuật

**Hỏi: Website bị lỗi hoặc không truy cập được thì liên hệ ai?**
Trả lời: Khách hàng liên hệ bộ phận hỗ trợ kỹ thuật qua hotline, email hoặc hệ thống ticket 24/7 để được kiểm tra và xử lý sự cố. Đội ngũ kỹ thuật sẽ phản hồi và khắc phục trong thời gian sớm nhất theo cam kết SLA của từng gói dịch vụ.

**Hỏi: Hosting có cam kết thời gian hoạt động (uptime) không?**
Trả lời: Có. Công ty cam kết uptime tối thiểu 99.9% cho tất cả các gói hosting. Nếu không đạt mức cam kết, khách hàng có thể được bồi hoàn theo chính sách SLA tương ứng.

## Tài liệu / liên kết liên quan

- [Bảng giá các gói hosting](đường-dẫn-hoặc-URL)
- [Hướng dẫn sử dụng bảng điều khiển hosting](đường-dẫn-hoặc-URL)
- [Chính sách bảo hành sản phẩm](chinh-sach-bao-hanh.md)
